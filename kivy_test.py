...
class EvoCanvasApp(App):
    ...
    def check_collision(self, p1, p2):
        ...
        #     return True
        if ((x1 - x2)**2 + (y1 - y2)**2)**0.5 <= (w1 + w2) / 2:
            return True
        return False
    ...
    def tick(self, dt):
        ...
        for p in self.particles:
            ...
            if (p.pos[0] > self.w) or (p.pos[0] < 0):
                p.vel[0] *= -1
            if (p.pos[1] > self.h) or (p.pos[1] < 0):
                p.vel[1] *= -1
    ...
    def add(self, n, *largs):
        self.particles += [Particle(rnd() * self.w, rnd() * self.h, self.speed, self.size) for _ in range(n)]
    ...
    def build(self):
        ...
        self.particles = [Particle(rnd() * self.w, rnd() * self.h, self.speed, self.size) for i in range(30)]
